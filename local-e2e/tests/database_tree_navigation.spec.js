...
test.describe('Database Tree Navigation', () => {
  ...
  test('DB를 클릭하면 선택되지만 로그인되지 않는다', async ({ page }) => {
    ...
    await expect(page.locator(`[data-testid="tab-db:${E2E_DB}"]`)).not.toBeVisible().catch(() => {});
  });
  ...
  test('DB를 더블 클릭하면 대시보드 탭이 열린다', async ({ page }) => {
    const dbTree = new DatabaseTreePage(page);
    await dbTree.activateDatabase(E2E_DB);

    await expect(page.getByText(E2E_DB, { exact: false }).first()).toBeVisible({ timeout: 15000 });
  });
