...
| `SEED` | 암호화 시드 — **첫 실행 시 자동 생성, 이후 수정 금지** | — |
| `SALT` | 암호화 솔트 — **첫 실행 시 자동 생성, 이후 수정 금지** | — |

> **주의**: `SEED`와 `SALT`는 저장된 모든 데이터의 암호화 키를 파생하는 데 사용됩니다.
> 첫 실행 시 자동 생성되어 `cwm.conf`에 저장되며, 이후 변경하면 기존 데이터를 복호화할 수 없습니다.
