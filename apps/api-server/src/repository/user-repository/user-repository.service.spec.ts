...
describe('UserRepositoryService', () => {
    ...
    describe('loadUserById', () => {
        it('should load user by id, decrypt and return User object', async () => {
            ...
            encryptionService.decryptValue.mockReturnValue(
                JSON.stringify(mockUser),
            );

            const result = await service.loadUserById(mockUserId);

            expect(encryptionService.getHashedValue).toHaveBeenCalledWith(
                mockUserId,
            );
        ...
        it('should throw error if user file not found', async () => {
            storageService.read.mockRejectedValue(
    ...
    describe('createUser', () => {
        it('should create a new user with hashed id and encrypted data', async () => {
            ...
            expect(passwordService.getHashedValue).toHaveBeenCalledWith(
                mockPassword,
            );
            expect(encryptionService.encryptValue).toHaveBeenCalled();
            expect(storageService.createAndWrite).toHaveBeenCalledWith(
                mockHashedId,
                mockEncryptedData,
            );
        });
    ...
    describe('updateUser', () => {
        ...
        it('should throw error if user file not found', async () => {
            storageService.write.mockRejectedValue(
                new Error('File not found'),
            );

            await expect(
                service.updateUser(mockUserId, mockUser),
            ).rejects.toThrow();
        });
    });
    ...
    describe('atomicUpdateUser', () => {
        it('should perform atomic update using lockService.withLock', async () => {
            ...
            const result = await service.atomicUpdateUser(
                mockUserId,
                modifierCallback,
            );
            ...
            expect(storageService.readUnsafe).toHaveBeenCalledWith(
                mockHashedId,
            );
            expect(encryptionService.decryptValue).toHaveBeenCalledWith(
                mockEncryptedData,
            ...
            expect(storageService.writeUnsafe).toHaveBeenCalledWith(
                mockHashedId,
                mockEncryptedData,
            );
        ...
        it('should throw error if read fails', async () => {
            ...
            storageService.readUnsafe.mockRejectedValue(
            ...
            await expect(
                service.atomicUpdateUser(mockUserId, modifierCallback),
            ).rejects.toThrow();
        ...
        it('should throw error if modifier callback fails', async () => {
            const modifierCallback = jest
                .fn()
                .mockRejectedValue(new Error('Modifier failed'));
