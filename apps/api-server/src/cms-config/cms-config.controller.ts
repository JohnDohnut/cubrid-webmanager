...
export class CmsConfigController {
  ...
  async addDbnameToServer(
    ...
    const response = await this.cmsConfigService.addDbnameToServerParam(userId, hostUid, body);
    return response;
  }
  ...
  async removeDbnameFromServer(
    ...
    const response = await this.cmsConfigService.removeDbnameFromServerParam(userId, hostUid, body);
    return response;
  }
}
