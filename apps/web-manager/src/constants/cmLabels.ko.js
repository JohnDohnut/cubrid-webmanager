...
export const CM_KO = {
  ...
  selectServerFromList: '위 목록에서 호스트를 선택하세요.',
  serverListAbove: '위 호스트 목록',
  admin: 'Admin',
  ...
  ipAddressDomainDescription: '지금 Next CUBRID Admin에 접속 중인 주소가 아니라, 관리하려는 CUBRID 호스트(CMS)가 설치된 장비의 주소입니다.',
  modifyHost: '호스트 수정',
  removeHostConnection: '호스트 연결 제거',
  keepHost: '유지',
  confirmRemoval: '제거 확인',
  activeConnectionsTerminated: '이 호스트에 대한 활성 연결이 종료됩니다.',
  changeManagerPasscode: '관리자 비밀번호 변경',
  newPasscode: '새 비밀번호',
  verifyNewPasscode: '비밀번호 확인',
  ...
  brokerParamsNotice: '브로커 매개변수는 Next CUBRID Admin이 CUBRID Broker와 통신하는 방식을 정의합니다. 이 변경은 이 호스트에 대한 Next CUBRID Admin의 연결에만 영향을 줍니다.',
  lockingSection: '잠금',
  ...
  webManagerConnectionDesc: (db) => `Next CUBRID Admin이 ${db}에 연결하는 방식을 구성합니다. 이 매개변수는 Next CUBRID Admin이 CUBRID Broker와 통신하는 방식을 정의합니다.`,
  ...
  workspaceDesktopOnlyDesc: '워크스페이스 설정은 Next CUBRID Admin 데스크톱 앱에서 사용할 수 있습니다.',
  ...
  importHostsDesc: 'Next CUBRID Admin 또는 CUBRID Admin 내보내기 파일에서 호스트를 가져옵니다.',
  ...
  importFormatHelp: 'Next CUBRID Admin XML/JSON, CUBRID Admin 호스트 XML, 또는 레거시 데스크톱 .prefs / .properties 파일입니다. 형식이 지원하는 경우 호스트 그룹도 보존됩니다. .prefs의 암호화된 비밀번호는 가져오지 않습니다.',
  ...
  weakNewPasswordMsg: '새 비밀번호가 정책을 충족하지 않습니다: 영문자와 숫자를 포함해 8자 이상 입력하세요.',
  currentPasswordIncorrectMsg: '현재 비밀번호가 올바르지 않습니다.',
  ...
  updatePasscode: '비밀번호 변경',
  passcodeChangedSuccessMsg: (host) => `${host}의 관리자 비밀번호 변경을 성공적으로 완료하였습니다.`,
  passcodeUpdateNote: '현재 로그인 중인 사용자의 CMS 계정 비밀번호를 변경합니다. 이 작업은 이 연결에 대해 로컬에 저장된 관리자 접속 비밀번호도 함께 업데이트합니다.',
