...
import {
  ...
  getDBUsersAPI,
  updateDBUserAPI,
} from '@/features/domain/DBUser/DBUserAPI.js';
  ...
  const updateUsers = async (server, db) => {
    ...
    if (responseUsers.status) {
      ...
      const newUsers = responseUsers.result.map((res) => {
        const parentId =
          createDBUser.type === 'add' ? createDBUser.node.key : createDBUser.node.parentId;
        return {
  ...
  return (
    ...
    >
      <Form form={form} layout="vertical" name="create_user_form">
        <Form.Item name="dbname" label="Database">
        ...
        <Form.Item name="groups" label="Groups">
          <Select mode="multiple" allowClear placeholder="Select user groups">
            {getRenderGroups()}
          </Select>
        </Form.Item>
      </Form>
