...
const CheckDB = () => {
  ...
  const handleOk = async () => {
    ...
    const response = await checkDBAPI(activeHost, {
      dbname: checkDB.node.title,
      repairdb: checked ? 'y' : 'n',
    }).finally(() => {
    ...
    >
      <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
        ...
        <div className={styles.db__layout}>
          ...
          <div>
            Verified a database. If inconsistencies are found, please contact CUBRID Support to help
            interpret the output and address any indicated problem so that it will not reoccur.
          </div>
        </div>
