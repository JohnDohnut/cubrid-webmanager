...
import {
  ...
  getDBSpaceAPI,
  loginDatabaseAPI,
} from '@/features/domain/database/databaseAPI.js';
...
const DeleteDB = () => {
  ...
  const handleOk = async () => {
    form.validateFields().then(async (values) => {
      ...
      const response = await loginDatabaseAPI(activeHost, {
        id: 'dba',
  ...
  useEffect(() => {
    if (deleteDB.open) {
      form.resetFields();
      dispatch(setBuffering(true));
      getDBSpaceAPI(activeHost, deleteDB.node)
        .then((res) => {
          if (res.success) {
    ...
    <>
      ...
      >
        ...
        <Checkbox
          ...
          onChange={({ target }) => setDeleteBackup(target.checked)}
        >
        ...
        <div style={{ overflowY: 'auto' }}>
          <div className={styles.db__layout}>
            <Form form={form} layout="horizontal" name="confirm_password_form">
              <Form.Item name="password" label="DBA Password">
                <Input.Password />
              </Form.Item>
            </Form>
          </div>
