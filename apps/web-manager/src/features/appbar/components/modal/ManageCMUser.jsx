...
import {
  ...
  getCMUsersAPI,
  updateCMUserAPI,
} from '@/features/domain/CMUser/CMUserAPI.js';
...
const EditableUserTable = () => {
  ...
  const setUserData = (data) => {
    setUsers(
      data.map((user) => ({
        ...
        statusMonitorAuth:
          typeof user.statusmonitorauth === 'object'
            ? user.statusmonitorauth?.auth_info
            : user.statusmonitorauth,
        key: nanoid(8),
      }))
    );
  };
  ...
  const handleSubmit = () => {
    form.validateFields().then(async (values) => {
      ...
      if (editingRecord) {
        ...
        dispatch(setBuffering(true));
        const response = await createCMUserAPI(activeHost, { ...setFormFormat(userData) }).finally(
          () => {
            dispatch(setBuffering(false));
          }
        );
  ...
  const columns = [
    { title: 'Username', dataIndex: 'targetId' },
    {
  ...
  return (
    ...
    >
      ...
      />
      <div style={{ marginTop: 16, display: 'flex', justifyContent: 'flex-end' }}></div>
